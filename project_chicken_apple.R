...
Apple_Simulation<- function(){
  ...
  scab_year_treatment <- (vv(yearly_scabcontrol_hours, var_CV, n=1) * hourly_wage +
                            cost_fungicide * orchard_area * 2/3 +
                            yearly_scabcontrol_hours *cost_diesel) * proportion_extracost_apple_scab  
  
  #what was the use the last 0.25? didn't look like it changed anything
  cost_scab_year <- chance_event(chance_intense_apple_scab,scab_year_treatment, 0, n=n_years)
  ...
  insect_year_treatment <- (yearly_insectcontrol_hours * hourly_wage +
                              cost_insecticide * orchard_area * 2/3 +
                              yearly_insectcontrol_hours * cost_diesel) * proportion_extracost_insect_infestation
  
  #draw chances that years are heavily infested by insects
  cost_insect_year<-chance_event(chance_intense_insect_infestation,insect_year_treatment, 0, n = n_years)
  ...
  cost_nutrients <- (vv(yearly_fertilization_hours, var_CV, n = n_years) * hourly_wage) + 
    cost_fertilizer * orchard_area * 2/3 +
  ...
  #cost to replace dead trees in a year
  ...
  occurence_vole_damage <- chance_event(chance_intense_vole_damage, 1, 0, n = n_years)
  
  #cost the voles create by damaging trees and having to replace them
  #ocurs only years when voles damage trees, otherwise zero
  cost_voles <- occurence_vole_damage * cost_replacement_trees
  ...
  #calculate the total apple yield
